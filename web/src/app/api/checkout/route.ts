...
export async function POST(req: NextRequest) {
  try {
    const { email, plan, priceVariant, ref, utm_source, utm_campaign } = await req.json();
    ...
    // Build request body with optional referral code, price variant, and UTM parameters
    const body: {
      email: string;
      plan: string;
      priceVariant?: string;
    ...
    if (priceVariant) {
      body.priceVariant = priceVariant;
    }
    if (ref) {
      body.ref = ref;
    }
